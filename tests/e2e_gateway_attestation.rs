...
use tameshi::collectors::ferrite::{
    FerriteCollector, PomsAnalysis, PomsArtifact, PomsHashes, PomsTarget,
};
...
async fn phase_a_safe_produces_signature() {
    let sig = FerriteCollector::new(safe_gateway_poms())
        .collect()
        .await
        .unwrap();
...
async fn phase_a_composes_into_merkle() {
    let ferrite = FerriteCollector::new(safe_gateway_poms())
        .collect()
        .await
        .unwrap();
    let nix = tameshi::signature::LayerSignature::new(
        LayerType::Nix,
        Blake3Hash::digest(b"nix"),
        "nix",
        vec![],
    );
    let oci = tameshi::signature::LayerSignature::new(
        LayerType::Oci,
...
async fn phase_a_deterministic() {
    let s1 = FerriteCollector::new(safe_gateway_poms())
        .collect()
        .await
        .unwrap();
    let s2 = FerriteCollector::new(safe_gateway_poms())
        .collect()
        .await
        .unwrap();
...
async fn phase_a_json_contract() {
    ...
    let sig = FerriteCollector::from_json(&json)
        .unwrap()
        .collect()
        .await
        .unwrap();
...
async fn phase_b_multi_violations() {
    ...
    assert!(
        FerriteCollector::from_json(&json)
            .unwrap_err()
            .to_string()
...
async fn phase_c_tamper_changes_root() {
    let genuine = FerriteCollector::new(safe_gateway_poms())
        .collect()
        .await
        .unwrap();
...
async fn phase_c_bit_flip() {
    let json = serde_json::to_vec(&safe_gateway_poms()).unwrap();
    let genuine = FerriteCollector::from_json(&json)
        .unwrap()
        .collect()
        .await
        .unwrap()
