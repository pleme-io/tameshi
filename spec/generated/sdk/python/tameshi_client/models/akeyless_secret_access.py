...
class AkeylessSecretAccess(BaseModel):
    """
    Record of a single Akeyless secret access
    """ # noqa: E501
    path: StrictStr = Field(description="Akeyless secret path")
    secret_type: AkeylessSecretType = Field(alias="secretType")
    value_hash: StrictStr = Field(description="BLAKE3 hash of the secret value (not the secret itself)", alias="valueHash")
    accessed_at: datetime = Field(description="Timestamp when the secret was accessed", alias="accessedAt")
    version: Optional[StrictInt] = Field(default=None, description="Secret version number")
    __properties: ClassVar[List[str]] = ["path", "secretType", "valueHash", "accessedAt", "version"]
